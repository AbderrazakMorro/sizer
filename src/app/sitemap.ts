import type { MetadataRoute } from "next";
import { getBlogSitemapPaths } from "@/lib/blog-data";
import { getSiteUrl } from "@/lib/site-url";

/**
 * Official sitemap: only /sitemap.xml is generated by Next.js (standard
 * sitemaps.org XML). robots.txt references this URL. Audit tools that report
 * "Unknown sitemap format" for other paths (e.g. sitemap_index.xml,
 * post-sitemap.xml) are probing common CMS URLs that this app does not serve.
 */
const publicRoutes = [
  { path: "/", priority: 1, changeFrequency: "weekly" as const },
  { path: "/en", priority: 1, changeFrequency: "weekly" as const },
  { path: "/tarifs", priority: 0.9, changeFrequency: "weekly" as const },
  { path: "/en/pricing", priority: 0.9, changeFrequency: "weekly" as const },
  {
    path: "/a-propos",
    priority: 0.8,
    changeFrequency: "monthly" as const,
  },
  {
    path: "/en/about-veta",
    priority: 0.8,
    changeFrequency: "monthly" as const,
  },
  { path: "/contact", priority: 0.8, changeFrequency: "monthly" as const },
  { path: "/en/contact", priority: 0.8, changeFrequency: "monthly" as const },
  { path: "/demo", priority: 0.8, changeFrequency: "monthly" as const },
  { path: "/en/demo", priority: 0.8, changeFrequency: "monthly" as const },
  { path: "/mentions-legales", priority: 0.5, changeFrequency: "yearly" as const },
  { path: "/en/legal", priority: 0.5, changeFrequency: "yearly" as const },
  {
    path: "/plan-de-base-premier-projet",
    priority: 0.7,
    changeFrequency: "monthly" as const,
  },
  {
    path: "/en/base-plan-first-interior-design-project",
    priority: 0.7,
    changeFrequency: "monthly" as const,
  },
  {
    path: "/plan-pro-independant",
    priority: 0.7,
    changeFrequency: "monthly" as const,
  },
  {
    path: "/en/pro-plan-for-independent-interior-designers",
    priority: 0.7,
    changeFrequency: "monthly" as const,
  },
  {
    path: "/plan-studio-entreprise",
    priority: 0.7,
    changeFrequency: "monthly" as const,
  },
  {
    path: "/en/studio-plan-for-architecture-and-interior-design-firms",
    priority: 0.7,
    changeFrequency: "monthly" as const,
  },
];

function blogSitemapPriority(path: string): number {
  if (path === "/blog" || path === "/en/blog") {
    return 0.75;
  }
  return 0.65;
}

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const baseUrl = getSiteUrl();
  const staticEntries = publicRoutes.map(
    ({ path, priority, changeFrequency }) => ({
      url: `${baseUrl}${path}`,
      lastModified: new Date(),
      changeFrequency,
      priority,
    })
  );

  const blogPaths = await getBlogSitemapPaths();
  const blogEntries = blogPaths.map(({ path, lastModified }) => ({
    url: `${baseUrl}${path}`,
    lastModified,
    changeFrequency: "monthly" as const,
    priority: blogSitemapPriority(path),
  }));

  return [...staticEntries, ...blogEntries];
}
